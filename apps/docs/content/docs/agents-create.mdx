...
For Codex agents, configure the shared Codex home at the daemon layer instead of `custom_env`: use `multica config set codex_home <path>`, `multica daemon start --codex-home <path>`, or the daemon process environment variable `CODEX_HOME`. The daemon still creates an isolated per-task `CODEX_HOME` for each run.
