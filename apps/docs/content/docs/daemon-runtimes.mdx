...
You can save local daemon startup defaults in the current CLI profile:
...
`daemon.codex_home` is the shared Codex home the daemon copies and links from when it creates each task's isolated `CODEX_HOME`. It is not the final per-task directory.

Daemon startup values resolve in this order:

1. Explicit `multica daemon start` flags (`--device-name`, `--workspaces-root`, `--codex-home`)
2. Environment variables (`MULTICA_DAEMON_DEVICE_NAME`, `MULTICA_WORKSPACES_ROOT`, `CODEX_HOME`)
3. Profile config (`daemon.device_name`, `daemon.workspaces_root`, `daemon.codex_home`)
4. Built-in defaults
...
| `multica daemon start --workspaces-root <path>` | Override the workspaces root for this daemon start |
| `multica daemon start --codex-home <path>` | Override the shared Codex home used to seed per-task `CODEX_HOME` |
