...
```bash
multica config set device_name home-pc
multica config set workspaces_root F:\ai-runtime\multica\workspaces
multica config set codex_home F:\ai-runtime\multica\codex-home
multica daemon restart
```

`codex_home` is the shared Codex home the daemon copies and links from when it creates each task's isolated `CODEX_HOME`. It is not the final per-task directory.
...
3. Profile config (`device_name`, `workspaces_root`, `codex_home`; legacy nested `daemon.*` keys are still read)
4. Built-in defaults
