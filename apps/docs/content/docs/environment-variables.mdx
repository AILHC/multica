...
| `MULTICA_WORKSPACES_ROOT` | `~/multica_workspaces` or profile-specific equivalent | Root directory for task working directories |
| `CODEX_HOME` | `~/.codex` | Shared Codex home used to seed each task's isolated `CODEX_HOME` |
...
```bash
multica config set workspaces_root /srv/multica/workspaces
multica config set codex_home /srv/multica/codex-home
...
| `workspaces_root` | `~/multica_workspaces` or profile-specific equivalent | Root directory for task working directories |
| `codex_home` | `~/.codex` | Shared Codex home used to seed per-task `CODEX_HOME` |
...
Legacy nested `daemon.workspaces_root` and `daemon.codex_home` values are still read. A subsequent `multica config set` writes the flat fields shown above.
