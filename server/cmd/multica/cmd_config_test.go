...
func TestApplyConfigSetMigratesLegacyDaemonKeysToFlatFields(t *testing.T) {
	...
	for key, value := range map[string]string{
		"daemon.device_name":     "flat-device",
		"daemon.workspaces_root": "/flat/workspaces",
		"daemon.codex_home":      "/flat/codex",
	} {
		if err := applyConfigSet(&cfg, key, value); err != nil {
			t.Fatalf("applyConfigSet(%s): %v", key, err)
		}
	}
	if cfg.DeviceName != "flat-device" ||
		cfg.WorkspacesRoot != "/flat/workspaces" ||
		cfg.CodexHome != "/flat/codex" {
		t.Fatalf("flat daemon values = %+v", cfg)
	}
	if cfg.Daemon != nil {
