...
type CLIConfig struct {
	...
	// WorkspacesRoot is the base directory used for daemon-managed task
	// environments. Legacy profiles stored this value under
	// daemon.workspaces_root; readers retain that fallback while new writes use
	// this flat field alongside the other persisted daemon knobs.
	WorkspacesRoot string `json:"workspaces_root,omitempty"`

	// CodexHome is the shared Codex home used to seed task-local CODEX_HOME
	// directories. Legacy profiles stored it under daemon.codex_home.
	...
	// Daemon retains the legacy nested daemon profile shape for read
	// compatibility. New writes use the flat fields above.
