...
func (d *Daemon) handleLocalSkillList(ctx context.Context, rt Runtime, requestID string) {
	...
	skills, supported, err := listRuntimeLocalSkills(rt.Provider, d.cfg.SharedCodexHome)
	if err != nil {
	...
	mcpServers, mcpSupported, err := listRuntimeLocalMcpServers(rt.Provider, d.cfg.SharedCodexHome)
	if err != nil {
...
func (d *Daemon) handleLocalSkillImport(ctx context.Context, rt Runtime, pending PendingLocalSkillImport) {
	...
	skill, supported, err := loadRuntimeLocalSkillBundle(rt.Provider, pending.SkillKey, d.cfg.SharedCodexHome)
	if err != nil {
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	if task.Agent != nil {
		...
		if merged, mergeErr := mergeRuntimeAndAgentMcpConfig(provider, d.cfg.SharedCodexHome, agentMcpConfig); mergeErr != nil {
	...
	if provider == "codex" {
		if store := execenv.CodexSessionStorePathAt(d.cfg.SharedCodexHome, d.cfg.Profile, task.AgentID, task.IssueID); store != "" {
	...
	if shouldReusePriorWorkdir(task, localAssignment, d.cfg.WorkspacesRoot) {
		...
		env, err = d.reuseExecutionEnvironment(prepareCtx, execenv.ReuseParams{
			...
			SharedCodexHome:       d.cfg.SharedCodexHome,
