...
type CodexHomeOptions struct {
	...
	GOOS string
	// SharedCodexHome overrides the source home used for task config, auth,
	// model/plugin caches, skills, and persistent issue session stores.
...
// Checks an explicit override first, then $CODEX_HOME, and falls back to
// ~/.codex.
...
func PruneCodexSessionStoresAt(sharedCodexHome, profile string, retention time.Duration, now time.Time, reserve func(storeDir string) (commit func(), ok bool), logger *slog.Logger) (removed int, bytesFreed int64) {
	...
	root := filepath.Join(resolveSharedCodexHome(sharedCodexHome), codexSessionStoreRoot, codexSessionStoreNamespace(profile))
...
func CodexSessionStorePath(profile, agentID, issueID string) string {
	return CodexSessionStorePathAt("", profile, agentID, issueID)
}

// CodexSessionStorePathAt returns the store path under an explicit shared
// Codex home.
func CodexSessionStorePathAt(sharedCodexHome, profile, agentID, issueID string) string {
	key := codexSessionStoreKey(profile, agentID, issueID)
	if key == "" {
		return ""
	}
	return codexSessionStoreDir(resolveSharedCodexHome(sharedCodexHome), key)
}
