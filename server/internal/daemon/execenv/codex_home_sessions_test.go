...
func TestPruneCodexSessionStores(t *testing.T) {
	...
	removed, bytes := PruneCodexSessionStoresAt(home, "", retention, now, nil, testLogger())
	if removed != 1 {
	...
	if removed, _ := PruneCodexSessionStoresAt(home, "", 0, now, nil, testLogger()); removed != 0 {
...
func TestPruneCodexSessionStores_ReopenedStoreNotReclaimed(t *testing.T) {
	...
	if removed, _ := PruneCodexSessionStoresAt(home, "", 14*24*time.Hour, time.Now(), nil, testLogger()); removed != 0 {
...
func TestPruneCodexSessionStores_ActiveStoreNotReclaimed(t *testing.T) {
	...
	if removed, _ := PruneCodexSessionStoresAt(home, "", 14*24*time.Hour, time.Now(), held, testLogger()); removed != 0 {
	...
	if removed, _ := PruneCodexSessionStoresAt(home, "", 14*24*time.Hour, time.Now(), nil, testLogger()); removed != 1 {
...
func TestPruneCodexSessionStores_IsolatesProfiles(t *testing.T) {
	...
	if removed, _ := PruneCodexSessionStoresAt(home, "staging", 14*24*time.Hour, time.Now(), nil, testLogger()); removed != 1 {
...
func TestPruneCodexSessionStores_NoCrossProfileCollision(t *testing.T) {
	for _, pair := range [][2]string{{"", "default"}, {"staging.prod", "stagingprod"}} {
		...
		if removed, _ := PruneCodexSessionStoresAt(home, a, 14*24*time.Hour, time.Now(), nil, testLogger()); removed != 1 {
		...
		if removed, _ := PruneCodexSessionStoresAt(home, b, 14*24*time.Hour, time.Now(), nil, testLogger()); removed != 1 {
...
func TestPruneCodexSessionStores_RemovesEmptyAgentDir(t *testing.T) {
	...
	if removed, _ := PruneCodexSessionStoresAt(home, "", 14*24*time.Hour, now, nil, testLogger()); removed != 1 {
