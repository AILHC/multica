...
type PrepareParams struct {
	...
	// SharedCodexHome is the explicit source for Codex config, credentials,
	// caches, skills, and issue session stores.
...
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	if params.Provider == "codex" {
		...
		if err := prepareCodexHomeWithOpts(codexHome, CodexHomeOptions{CodexVersion: params.CodexVersion, SharedCodexHome: params.SharedCodexHome, IsLocalDirectory: params.LocalWorkDir != "" || params.LocalWorktree != nil, SessionStoreKey: codexSessionStoreKey(params.Profile, params.Task), CodexCustomArgs: params.CodexCustomArgs}, logger); err != nil {
			return nil, fmt.Errorf("execenv: prepare codex-home: %w", err)
		}
		if err := hydrateCodexSkills(codexHome, params.SharedCodexHome, params.Task.AgentSkills, params.Task.DisabledRuntimeSkills, logger); err != nil {
...
type ReuseParams struct {
	...
	SharedCodexHome string // explicit source home for Codex task state
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	if params.Provider == "codex" {
		...
		if err := prepareCodexHomeWithOpts(codexHome, CodexHomeOptions{CodexVersion: params.CodexVersion, SharedCodexHome: params.SharedCodexHome, ResumeSessionID: params.ResumeSessionID, IsLocalDirectory: params.LocalDirectory, SessionStoreKey: codexSessionStoreKey(params.Profile, params.Task), CodexCustomArgs: params.CodexCustomArgs}, logger); err != nil {
...
func hydrateCodexSkills(codexHome, sharedCodexHome string, workspaceSkills []SkillContextForEnv, disabledRuntimeSkills []RuntimeSkillRefForEnv, logger *slog.Logger) error {
