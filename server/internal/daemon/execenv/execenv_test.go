...
func TestCodexSessionStoreAndPruneUseSharedCodexHomeParam(t *testing.T) {
	sharedHome := t.TempDir()
	envHome := t.TempDir()
	t.Setenv("CODEX_HOME", envHome)

	store := CodexSessionStorePathAt(sharedHome, "", TaskContextForEnv{AgentID: "agent-1", IssueID: "issue-1"})
	if !strings.HasPrefix(store, filepath.Join(sharedHome, codexSessionStoreRoot)+string(os.PathSeparator)) {
		t.Fatalf("store = %q, want it under explicit shared home %q", store, sharedHome)
	}
	if err := os.MkdirAll(store, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(store, "rollout.jsonl"), []byte("{}"), 0o644); err != nil {
		t.Fatal(err)
	}

	removed, _ := PruneCodexSessionStoresAt(sharedHome, "", 24*time.Hour, time.Now().Add(48*time.Hour), nil, testLogger())
	if removed != 1 {
		t.Fatalf("removed = %d, want 1", removed)
	}
	if _, err := os.Stat(store); !os.IsNotExist(err) {
		t.Fatalf("store still exists after prune: %v", err)
	}
}
...
func TestPrepareCodexWorkspaceSkillBeatsUserSkillOnConflict(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("read task config: %v", err)
	}
	if strings.Contains(string(config), "[[skills.config]]") {
		t.Fatalf("workspace skill was disabled by a same-key runtime policy:\n%s", config)
	}
}
...
func TestWriteEnvRootOwnerAtomicallyReplacesMarker(t *testing.T) {
	...
	// Windows resolves file IDs lazily from the stored path. Cache the old
	// identity before the rename so SameFile does not reopen the replacement.
	if !os.SameFile(before, before) {
		t.Fatal("could not resolve legacy owner file identity")
	}
