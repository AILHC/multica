...
func TestPreparationRequestPreservesSharedCodexHomeForHelper(t *testing.T) {
	t.Parallel()

	want := filepath.Join(t.TempDir(), "profile-codex-home")
	tests := []preparationRequest{
		{Action: preparationActionPrepare, Prepare: &PrepareParams{SharedCodexHome: want}},
		{Action: preparationActionReuse, Reuse: &ReuseParams{SharedCodexHome: want}},
	}
	for _, request := range tests {
		...
		if got.Prepare != nil && got.Prepare.SharedCodexHome != want {
			t.Fatalf("prepare shared home = %q, want %q", got.Prepare.SharedCodexHome, want)
		}
		if got.Reuse != nil && got.Reuse.SharedCodexHome != want {
			t.Fatalf("reuse shared home = %q, want %q", got.Reuse.SharedCodexHome, want)
		}
	}
}
